...
# read data
SERIAL_PORT = '/dev/tty.usbmodem142401'
SERIAL_RATE = 115200  # refers to Arduino IDE

ser = serial.Serial(SERIAL_PORT, SERIAL_RATE)  # define serial
all_data = []
...
savetxt('data.csv', final_data, delimiter=',')
...
axs[0].set_ylabel('Rote & Infrarote Amplitude')
...
# run filter on signals
filtered_red_vec2 = lfilter(taps02, 1.0, red_vec)
filtered_ir_vec2 = lfilter(taps02, 1.0, ir_vec)

sub_plots_filtered(1, 0, xlp, filtered_red_vec2, 'FIR Bandpassfilterung / Wechselanteil')
...
I_RACDC = np.divide(filtered_red_vec2, filtered_red_vec1) * (-1)
...
plt.savefig('ACDCnorm.png', dpi=300, facecolor=[252 / 255, 252 / 255, 252 / 255])
plt.show()

final_NORMdata = [xlp, I_RACDC, I_IRACDC, R]

# save normalized signals and r-value as NORMdata.csv
savetxt('NORMdata.csv', final_NORMdata, delimiter=',')
