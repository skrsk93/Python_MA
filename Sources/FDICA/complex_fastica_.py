...
def _ica_par(X, tol, g, fun_args, max_iter, w_init):
    ...
    for ii in range(max_iter):
        ...
        W1 = _sym_decorrelation(np.dot(gwtx * U, np.conj(X.T)) / p_ - g_wtx[:, np.newaxis] * W)
        
        del gwtx, g_wtx
        # builtin max, abs are faster than numpy counter parts.
        lim = max(abs(abs(np.diag(np.dot(W1, np.conj(W.T)))) - 1))
        W = W1
        if lim < tol:
            break
    else:
...
def complexfastica(X, n_components=None, algorithm="parallel", whiten=True,
            fun="logcosh", fun_args=None, max_iter=1000, tol=1e-06, w_init=None,
...
class ComplexFastICA(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, n_components=None, algorithm='parallel', whiten=True,
        ...
        super(ComplexFastICA, self).__init__()
    ...
    def _fit(self, X, compute_sources=False):
        ...
        whitening, unmixing, sources, X_mean, self.n_iter_ = complexfastica(
